...
- 9ca43ae: Typing, deleting and pasting inside a table cell now go through the ops API.
...
- 5abf030: Fix a vertically aligned page rendering top-aligned after a reload. A document
...
- 9f488ba: A paragraph-boundary Backspace / Delete (a paragraph MERGE) now runs
...
- 9a6f61f: Table cells (and paragraphs / runs) with a PATTERN shading now show the
...
- 2d0e43a: Architecture: extract the pure inline/range and revision/comment mutation
...
- 5c65a72: Dot-leader TOC / list-of-\* lines now render Word-faithfully: the page
...
- 6bfa5d5: Paragraph-mark run properties (`<w:pPr><w:rPr>`) no longer resize a
...
- 66f4e7f: Paragraph-anchored frames measure from the paragraph's TOP again —
...
- 8c7a2f9: Paragraph-anchored floating objects hang from the anchor line's
...
- 94ee70b: Lato line heights use the font's usWin metrics (1.439×) — Word and
...
- c160ef2: `<w:tblLayout w:type="fixed"/>` dxa tables lay out on Word's column
...
- 25d35ad: One flow-rendering contract everywhere: the body, header, footer,
...
- 54179cf: Tab-spread layout applies inside header/footer zones — the flex rules
...
- 8bf75ac: Form-document fidelity, four fixes: (1) `<w:sym>` symbol-font glyphs
...
- d15b17c: Table styles' own paragraph/run defaults now apply to every paragraph
...
- 07e477d: Honest page budgets for re-plan tail pages, and trailing blank pages
...
- 23f300d: Honour `<w:contextualSpacing/>` so runs of same-style paragraphs render
