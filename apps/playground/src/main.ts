 ...
 * the same content (e.g. editing inside the two-column section + undo).
 */
function tryDocSeed(): SobreeDocument {
  const ACCENT = "#F2A900";
  const INK = "#1F1B16";
  const MUTED = "#6B6256";
  const BAND = "#FCF3DD";
  ...
  const head = (
    ...
    paragraph([text(label, { bold: true, fontSizePt, color: INK, fontFamily: SERIF })], {
      spacing,
    });
